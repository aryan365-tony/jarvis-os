...
3. accumulates OpenAI-style streaming ``tool_calls``; when the model asks for a
   tool it runs it through the registry (with confirmation + budget), appends the
   result, and continues the turn;
4. enforces ``max_tool_calls_per_turn`` and a per-turn step cap so a runaway
   model can never hang the interface.
...
# Confirmation prompt for irreversible tools.
...
class ConversationAgent:
    ...
    async def send(self, user_text: str, on_delta: DeltaFn) -> str:
        ...
        for _step in range(self._cfg.policy.max_turns_per_session_task):
            ...
            for tc in tool_calls:
                ...
                result = await execute(
                    name, args, confirm=self._confirm, call_counts=call_counts
                )
                if self._on_task:
